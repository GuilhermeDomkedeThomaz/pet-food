    ...
    public ResponseEntity<?> getUser(String name) {
        ...
        if (user.isPresent()) {
            ...
            try {
                ...
                return new ResponseEntity<>("Erro no mapeamento para retorno do usuário: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } else
            ...
            if (Objects.nonNull(passwordEncrypted)) {
                try {
                    ...
                    try {
                        ...
                        return new ResponseEntity<>("Erro ao atualizar usuário na base de dados: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
                    }
                } catch (Exception e) {
                    return new ResponseEntity<>("Erro no mapeamento para atualização do usuário: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
                }
            } else
                return new ResponseEntity<>("Erro ao gerar senha criptografada para o usuário.", HttpStatus.INTERNAL_SERVER_ERROR);
        } else
    ...
    public ResponseEntity<?> deleteUser(String name) {
        ...
        if (user.isPresent()) {
            ...
            try {
                ...
                return new ResponseEntity<>("Erro ao deletar usuário na base de dados: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } else
            return new ResponseEntity<>("Usuário não encontrado.", HttpStatus.BAD_REQUEST);
    }
